...
randPassNoCapitalLetter = ''.join(random.choices(string.ascii_lowercase + string.digits, k = 7))
password.send_keys(randPassNoCapitalLetter) #enter password in text field
passvalue = password.get_attribute('value')


# confirm password
confirmpassword = driver.find_element_by_name('confirmpassword')
confirmpassword.send_keys(randPassNoCapitalLetter) #enter the same password entered before in password text field
